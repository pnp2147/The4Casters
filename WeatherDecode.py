# -*- coding: utf-8 -*-
"""
Created on Sun Feb 25 21:54:42 2024
...
def weather_decode(wt_str):
    ...
    if (not isinstance(wt_str, str)) and math.isnan(wt_str):
        return wt_str
    ...
    for i in range(len(wt_typ)):
        current = wt_typ[i]
        flag = AU.__contains__(current) | AW.__contains__(current) | MW.__contains__(current)
        wt_bit += str(int(flag))
...
df[wt] = df[wt].apply(lambda x: weather_decode(x))

print(df[wt])

df.drop('Unnamed: 0', axis = 1, inplace = True)

df.to_csv("master_wt.csv", index = False)